...
function _makeMessageChunkFromAnthropicEvent(data, fields) {
	...
	if (data.type === "message_start") {
		...
		const responseMetadata = require_index.iife(() => {
			const output = {};
			if ("context_management" in data.delta) output["context_management"] = data.delta.context_management;
			if ("usage" in data && "cost" in data.usage && typeof data.usage.cost === "number") output["usage"] = { cost: data.usage.cost };
			return output;
		});
		return { chunk: new _langchain_core_messages.AIMessageChunk({
			content: fields.coerceContentToString ? "" : [],
		...
		if (fields.coerceContentToString && "text" in data.delta) return { chunk: new _langchain_core_messages.AIMessageChunk({ content: data.delta.text }) };
		else {
