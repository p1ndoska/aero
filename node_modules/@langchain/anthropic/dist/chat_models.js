...
import { ANTHROPIC_TOOL_BETAS, AnthropicToolExtrasSchema, getTopLevelSchemaCompositionKeys, handleToolChoice } from "./utils/tools.js";
import { _convertMessagesToAnthropicPayload } from "./utils/message_inputs.js";
import { getSamplingParams, getTaskBudgetBetas, validateInvocationParamCompatibility } from "./utils/params.js";
...
var ChatAnthropicMessages = class extends BaseChatModel {
	...
	*/
	...
	constructor(modelOrFields, fieldsArg) {
		...
		super(fields ?? {});
		this._addVersion("@langchain/anthropic", "1.5.9");
		const gatewayConfig = resolveLangSmithGatewayConfig({
			baseURL: fields.anthropicApiUrl ?? fields.clientOptions?.baseURL ?? (getEnvironmentVariable("ANTHROPIC_API_URL") || getEnvironmentVariable("ANTHROPIC_BASE_URL") || void 0),
			providerPath: "anthropic"
		});
		this.anthropicApiKey = fields.apiKey ?? fields.anthropicApiKey ?? gatewayConfig.apiKey ?? getEnvironmentVariable("ANTHROPIC_API_KEY");
	...
	formatStructuredToolToAnthropic(tools, fields) {
		...
		return tools.map((tool) => {
			...
			if (isLangChainTool(tool)) {
				const { strict: extrasStrict, ...restExtras } = tool.extras ? AnthropicToolExtrasSchema.parse(tool.extras) : {};
			...
			if (!isAnthropicTool(tool)) return true;
			if (getTopLevelSchemaCompositionKeys(tool).length === 0) return true;
			return false;
		});
	}
	...
	*/
	invocationParams(options) {
		...
		const tools = this.formatStructuredToolToAnthropic(options?.tools, { strict: options?.strict });
		if (tool_choice?.type === "tool" && !tools?.some((tool) => "name" in tool && tool.name === tool_choice.name)) throw new Error(`Anthropic tool_choice references "${tool_choice.name}", but that tool is not available.`);
		if (tool_choice?.type === "any" && (!tools || tools.length === 0)) throw new Error("Anthropic tool_choice requires at least one available tool.");
		const output = {
			model: this.model,
		...
		Object.assign(output, getSamplingParams({
			model: this.model,
			thinking: this.thinking,
			topK: this.topK,
			topP: this.topP,
	...
	_getInvocationParamsForTracing(options) {
		const params = this.invocationParams(options);
		if (!Array.isArray(params.mcp_servers)) return params;
		return {
			...params,
			mcp_servers: params.mcp_servers.map((server) => {
				if (!("authorization_token" in server)) return server;
				return {
					...server,
	...
	_getCallOptionsForTracing(options, _invocationParams) {
		if (!Array.isArray(options.mcp_servers)) return options;
		return {
			...options,
	...
	*/
	async *_streamChatModelEvents(messages, options, _runManager) {
		...
		yield* convertAnthropicStream(abortableStream(stream, options.signal), { streamUsage: shouldStreamUsage ?? true });
	}
	/** @ignore */
	...
	*/
	async createStreamWithRetry(request, options) {
		...
		return this.caller.callWithOptions({ maxRetries: options?.maxRetries }, makeCompletionRequest);
	}
	/** @ignore */
