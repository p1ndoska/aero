...
import { useGetSecurityPolicyPageContentQuery, useUpdateSecurityPolicyPageContentMutation } from '@/app/services/securityPolicyPageContentApi';
...
export default function SecurityPolicyPage() {
  ...
  const { data: pageContent, refetch: refetchPageContent } = useGetSecurityPolicyPageContentQuery();
  const [updatePageContent, { isLoading: isUpdatingContent }] = useUpdateSecurityPolicyPageContentMutation();
  ...
  const handleOpenContentEditor = () => {
    if (pageContent) {
      setEditableTitle(getTranslatedField(pageContent, 'title', language) || 'Политика безопасности');
      ...
      setEditableTitle('Политика безопасности');
      setEditableSubtitle('');
      setEditableContent([]);
    }
        ...
        return (
          <div className="mb-6 flex flex-col items-center">
            <img 
              ...
              onError={(e) => {
                console.error('Image failed to load in SecurityPolicyPage:', element.props?.src);
                e.currentTarget.style.display = 'none';
              }}
              onLoad={() => {
                console.log('Image loaded successfully in SecurityPolicyPage:', element.props?.src);
              }}
            />
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
          ...
          <div className="text-center mb-12">
            <div className="flex items-center justify-center gap-3 mb-4">
              <h1 className="text-4xl font-bold text-gray-900 flex items-center gap-3">
                ...
                {getTranslatedField(pageContent, 'title', language) || 'Политика безопасности'}
              </h1>
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              {getTranslatedField(pageContent, 'subtitle', language) || 'Политика безопасности и конфиденциальности данных в нашем предприятии. Принципы защиты информации и обеспечения безопасности.'}
            </p>
          </div>
          ...
          {(!pageContent?.content || pageContent.content.length === 0) && (
            <div className="w-full">
              <div className="bg-blue-50 py-12 text-center rounded-lg">
                ...
                <h3 className="text-xl font-semibold text-gray-700 mb-2">Политика безопасности</h3>
                <p className="text-gray-500 mb-6">
                  Здесь будет размещена информация о политике безопасности предприятия.
                </p>
                {isAuthenticated && isAdmin && (
                  <Button
                    onClick={handleOpenContentEditor}
      ...
      <Dialog open={isContentEditorOpen} onOpenChange={setIsContentEditorOpen}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto bg-white min-w-0 dialog-content">
          <DialogHeader>
            <DialogTitle>Управление контентом страницы политики безопасности</DialogTitle>
            <DialogDescription>
              Редактируйте заголовок, подзаголовок и основной контент страницы политики безопасности.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6">
            <div>
              ...
              <Input
                ...
                placeholder="Политика безопасности"
              />
            </div>
            <div>
              ...
              <Textarea
                ...
                placeholder="Описание политики безопасности..."
